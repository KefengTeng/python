...
# 建表
c.execute('CREATE TABLE IF NOT EXISTS snmp_group (ip, ro, rw, groupno)')

# 清空表数据
c.execute('DELETE FROM snmp_group')

# 插入数据
device = []
with open(r'/tmp/snmp.txt', 'r') as f:
    # 列表解析, 去掉换行符
...
c.executemany('INSERT INTO snmp_group VALUES (?, ?, ?, ?)', device)

# 提交
conn.commit()

# 读取分组
c.execute('SELECT DISTINCT groupno FROM snmp_group')
rows = c.fetchall()
...
# 调用进程
for row in rows:
    with open(r'/tmp/' + f'snmp_{row[0]}.txt', 'a') as f:
