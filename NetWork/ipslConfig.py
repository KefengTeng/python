...
import pypinyin, paramiko, os, re
import pandas as pd
...
fileList.sort(key=lambda file: os.path.getmtime(excelDir + file) if os.path.isfile(excelDir + file) else 0)
print('\n当前待读取Excel文件: %s\n' % fileList[-1])
...
custEnName = "".join(map(lambda lowerEn: lowerEn.capitalize(), pypinyin.lazy_pinyin(paraDict['客户名称'])))
servNo = paraDict['电路代号']
servQos = paraDict['速率(M)']
servVlan = paraDict['VLAN']
interNet, interMask = paraDict['互联IP'].split('/')
interIP1, interIP2 = ".".join(interNet.split('.')[0:3]) + '.' + str(int(interNet.split('.')[3]) + 1), ".".join(interNet.split('.')[0:3]) + '.' + str(int(interNet.split('.')[3]) + 2)
binInterMask = "255." * int(int(interMask)//8) + str(int(256 - 256/2**(int(interMask) % 8)))
custNet = ''
if '用户IP' in paraDict:
    custNet, custMask = paraDict['用户IP'].split('/')
    binCustMask = "255." * int(int(interMask)//8) + str(int(256 - 256/2**(int(custMask) % 8)))
srDev = paraDict['SR设备'] if 'SR设备' in paraDict else ''
srIp = re.search(r'(\d+\.\d+\.\d+\.\d+)', srDev).group(1) if srDev != '' else ''
...
print('car cir %d pir %d inbound' % (int(servQos) * 1024 * 1.15, int(servQos) * 1024 * 1.15))
print('car cir %d pir %d outbound' % (int(servQos) * 1024 * 1.15, int(servQos) * 1024 * 1.15))
print('quit\n')
print('###接口及路由配置###\n')
if srDownPort != '':
...
print('qos-profile uni_%sm inbound vlan %s identifier none' % (servQos, servVlan))
print('qos-profile uni_%sm outbound vlan %s identifier none' % (servQos, servVlan))
...
if srIp != '' and srDownPort != '' and swIp != '' and swDownPort != '':
    ...
    interfaceSrCmdList = ConfigDict.setdefault(srIp, {}).setdefault('interface-config', [])
    routeSrCmdList = ConfigDict.setdefault(srIp, {}).setdefault('route-config', [])
    swCmdList = ConfigDict.setdefault(swIp, [])
    qosSrCmdList.append('qos-profile uni_%sm' % (servQos))
    qosSrCmdList.append('car cir %d pir %d inbound' % (int(servQos) * 1024 * 1.15, int(servQos) * 1024 * 1.15))
    qosSrCmdList.append('car cir %d pir %d outbound' % (int(servQos) * 1024 * 1.15, int(servQos) * 1024 * 1.15))
    qosSrCmdList.append('quit')
    interfaceSrCmdList.append('interface %s.%s0000' % (srDownPort, servVlan))
    interfaceSrCmdList.append('description USER::%s::%s.CO::%sM' % (servNo, custEnName, servQos))
    ...
    interfaceSrCmdList.append('qos-profile uni_%sm inbound vlan %s identifier none' % (servQos, servVlan))
    interfaceSrCmdList.append('qos-profile uni_%sm outbound vlan %s identifier none' % (servQos, servVlan))
